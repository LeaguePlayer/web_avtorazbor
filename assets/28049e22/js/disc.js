...
$(function(){
	...
	$('#sort li a').on('click',function(){
		...
		$('#Search_sort').val($(this).closest('li').data('sort'));
	...
	$('#display li a').on('click',function(){
		...
		$('#Search_display').val(parseInt($(this).text()));
	...
	$('#slider .calculate').slider({
		...
		min:14,
		max:25,
		values:[parseInt($('#Search_diametr_st').val(),10),parseInt($('#Search_diametr_end').val(),10)],
		slide:function(event, ui){

			$_max=$('#Search_diametr_end');
			$_min=$('#Search_diametr_st');
	...
	$('#slider-2 .calculate').slider({
		...
		slide:function(event, ui){
			...
			$_max=$('#Search_price_end');
			$_min=$('#Search_price_st');
