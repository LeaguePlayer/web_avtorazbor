...
<script>
	...
	//change price
	jQuery('.requests').on('blur', '.change-price', function(){
		var part_id = jQuery(this).data('id'),
			price = jQuery(this).val(),
			req_id = jQuery('.requests').data('request-id');

		if(price > 0 && typeof part_id === 'number'){
			jQuery.ajax({
				url: '<?=$this->createUrl("changePrice")?>',
				data: {request_id: req_id, part_id: part_id, price: price},
				success: function(data){
					jQuery('.parts-update').html(data);
				}
			});
